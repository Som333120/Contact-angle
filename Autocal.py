...
def findconner(image_input,color_input ) :
    ...
    corners = cv2.goodFeaturesToTrack(image_input, 6, 0.01, 10)
    ...
    print(len(contact_point))
    return color_input

def selectPoint(point1,point2,image_drawing_input) :
...
image = cv2.imread('ti.png',0) #read image from directery

blur = cv2.GaussianBlur(image,(3,3),0)
r = cv2.selectROI("Select Area",blur) #select ROI to cut image 
...
while (1) :
    ...
    RGB_convert =cv2.cvtColor(bright,cv2.COLOR_GRAY2BGR) 
    xx = findconner(edge,RGB_convert ) 
    ...
    cv2.imshow("find_edge",xx)   
    cv2.waitKey(1)
    
    point1s = int(input("Enter_Point 1 : "))
    point2s = int(input("Enter_Point 2 : "))
    selectPoint(point1 = point1s,point2 =point2s,image_drawing_input = RGB_convert)
